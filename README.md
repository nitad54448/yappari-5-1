...
There is no need for a circuit model for the DRT calculations : this procedure can help the user designing the electrical circuit. The usefulness of DRT depends much on the quality of the data.
On the DRT graph, the experimental Zr and Zi are plotted together with the reconstructed impedances from the DRT data and a probability of distribution function. Calculations are made in real time if there is an adjustement of the Tikhonov parameter for all selected data sets therefore, if you have several datasets, it may be slow. 
