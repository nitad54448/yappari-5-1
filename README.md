...
#### DRT active datasets ####
This performs a calculation of Distribution of Relaxation Times for one or more datasets. The method used is constrained non-negative linear regression with a Tikhonov parameter. The procedure used was described and coded by [Christian Altenbach](https://sites.google.com/site/altenbach/Home) which developed it for [EPR spectroscopy programs](https://ieprs.org/newsletter/vol-31-2/). This method is very fast and therefore it is possible to search an optimal regularization parameter, see below.
